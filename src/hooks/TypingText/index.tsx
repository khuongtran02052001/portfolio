...
export const useTypingText = (words: string[], keySpeed = 1000, maxPauseAmount = 10) => {
  ...
  const [currentWord, setCurrentWord] = React.useState(words[wordIndex].split(""));
  const [isStopped, setIsStopped] = React.useState(false);
  const direction: any = React.useRef(BACKWARD);
  const typingInterval: any = React.useRef();
  const letterIndex: any = React.useRef();
